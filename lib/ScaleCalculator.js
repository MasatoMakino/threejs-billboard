...
class ScaleCalculator {
    ...
    initRenderTarget(scene) {
        const geo = new three_1.SphereBufferGeometry(1e-4, 3, 2);
        const mat = new three_1.MeshBasicMaterial({
            transparent: true,
            opacity: 0.0,
            depthTest: false,
        });
        const renderTarget = new three_1.Mesh(geo, mat);
    ...
    getDotByDotScale(target) {
        const size = this._renderer.getSize(new three_1.Vector2());
    ...
    getFovHeight(distance) {
        const halfFov = three_1.MathUtils.degToRad(this._camera.fov / 2);
