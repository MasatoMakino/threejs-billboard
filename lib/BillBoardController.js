...
var BillBoardController = /** @class */ (function () {
    ...
    function BillBoardController(target, url, imageScale, option) {
        ...
        new three_6.TextureLoader().load(url, function (texture) {
    ...
    BillBoardController.prototype.getMaterial = function (target) {
        if (target instanceof three_3.Mesh) {
            return new three_4.MeshBasicMaterial({
                blending: three_2.NormalBlending,
                depthTest: true,
                transparent: true
            });
        }
        if (target instanceof three_3.Sprite) {
    ...
    BillBoardController.prototype.initDummyPlane = function (target) {
        if (target instanceof three_3.Mesh) {
            var size = 0.0000001;
            target.geometry = new three_5.PlaneBufferGeometry(size, size);
        }
    };
    BillBoardController.prototype.initGeometry = function (image) {
        if (!(this._target instanceof three_3.Mesh))
            return;
        if (this.isInitGeometry)
            return;
        this._target.geometry = new three_5.PlaneBufferGeometry(image.width, image.height);
        this.isInitGeometry = true;
    };
    BillBoardController.prototype.calculateScale = function (img) {
        if (this._target instanceof three_3.Sprite) {
            return {
